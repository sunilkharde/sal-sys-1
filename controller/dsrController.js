...
class dsrController {
    ...
    static viewPM = async (req, res) => {
        try {
            ...
            res.render('dsr/dsr-view-pm', { layout: 'mobile', data: results[0], data2: results2[0], dataNote, showNote: showNote });
    ...
    static update = async (req, res) => {
        ...
        try {
            ...
            const sqlStr = "UPDATE dsr_1 Set atten_flag=?,hr_flag=?,from_city=?,to_city=?,stay_city=?,total_allow=?,total_lodge=?,total_exp=?,u_at=CURRENT_TIMESTAMP,u_by=?" +
                " WHERE dsr_date=? and emp_id=?"
            const params = [atten_flag, hr_flag, from_city, to_city, stay_city, total_allow, total_lodge, total_exp, u_by, dsr_date, emp_id];
            await executeQuery(sqlStr, params);
